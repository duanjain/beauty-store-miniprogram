Page({
    ...
    wx.cloud.callFunction({
      ...
      success: res => {
        ...
        if (res.result.success) {
          that.setData({
            service: res.result.data
          });
        } else {
          wx.showToast({
            title: res.result.message || '获取服务详情失败',
            icon: 'none'
          });
          wx.navigateBack();
        }
  ...
  // 判断某个营业日是否被选中
  isWeekDayChecked(dayId) {
    return this.data.service.timeSettings.availableDays.includes(dayId);
  },

  // 每周营业日变化
  onWeekDayChange(e) {
    const { value } = e.detail;
    console.log('onWeekDayChange事件触发，e.detail.value:', value);
    // 将字符串转换为数字
  ...
  // 提交表单
  submitForm() {
    const { serviceId, service } = this.data;
    // 简单的表单验证
    ...
    const serviceData = {
      ...
      timeSettings: {
        ...timeSettings,
        timeGranularity: Number(timeSettings.timeGranularity),
        maxCapacity: Number(timeSettings.maxCapacity)
      }
    };
